...
func warn(kind, packageID, message string) models.SortWarning {
	return models.SortWarning{Kind: kind, PackageID: models.Str(packageID), Message: message}
}
...
func Sort(activeIDs []string) (models.SortResult, error) {
	p := paths.Detect()
	installed := ScanAll(models.Deref(p.GameInstall), p.WorkshopDirs)
	rules := RulesForSort()
	return SortWith(activeIDs, installed, rules, models.Deref(p.GameVersion)), nil
}
