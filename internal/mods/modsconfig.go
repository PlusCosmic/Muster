...
func ParseModsConfig(body string) (models.ActiveModList, error) {
	...
	for _, node := range root.Children {
		...
		case "version":
			list.Version = models.Str(strings.TrimSpace(node.Text()))
		case "activemods":
...
func RenderModsConfig(list models.ActiveModList) string {
	...
	if v := models.Deref(list.Version); v != "" {
...
func environment() (version string, knownExpansions []string) {
	p := paths.Detect()
	mods := ScanAll(models.Deref(p.GameInstall), p.WorkshopDirs)
	return models.Deref(p.GameVersion), InstalledExpansions(mods)
}
...
func GetActive(profileID string) (models.ActiveModList, error) {
	list, err := ReadActive(ModsConfigPath(appdir.ProfileDir(profileID)))
	if err != nil {
...
func SetActive(profileID string, activeIDs []string) error {
	path := ModsConfigPath(appdir.ProfileDir(profileID))
	...
	if len(known) == 0 && existing != nil {
		...
		log.Printf("rimforge: no official expansions detected; keeping %v", known)
	}
	...
	return WriteActive(path, models.ActiveModList{
		...
		KnownExpansions: models.NonNil(known),
		Version:         models.Str(version),
	})
}
