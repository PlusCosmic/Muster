// Package paths detects external locations (Steam, the RimWorld install,
// workshop content, the default savedata folder).
//
...
func Detect() models.DetectedPaths {
	...
	steamRoot := models.Deref(s.SteamRootOverride)
	if steamRoot == "" {
	...
	gameInstall := models.Deref(s.GameInstallOverride)
	if gameInstall == "" {
	...
	defaultSavedata := models.Deref(s.DefaultSavedataOverride)
	if defaultSavedata == "" {
	...
	return models.DetectedPaths{
		SteamRoot:       models.Str(steamRoot),
		GameInstall:     models.Str(gameInstall),
		DefaultSavedata: models.Str(defaultSavedata),
		WorkshopDirs:    workshopDirs,
		GameVersion:     models.Str(gameVersion),
		ProfilesDir:     appdir.ProfilesRoot(),
	}
}
...
func home() string {
	...
	if err != nil {
		log.Printf("rimforge: no home directory: %v", err)
		return ""
	}
	return h
}
