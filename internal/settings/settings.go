...
func Load() models.Settings {
	...
	if err != nil {
		if !errors.Is(err, os.ErrNotExist) {
			log.Printf("rimforge: could not read %s: %v", path, err)
		}
		return models.Settings{}
	}
	var s models.Settings
	if err := json.Unmarshal(raw, &s); err != nil {
		log.Printf("rimforge: malformed %s: %v", path, err)
		return models.Settings{}
	}
	return Normalize(s)
}

// Save writes settings.json atomically, creating `<data>/rimforge/` if needed.
