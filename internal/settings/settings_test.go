...
func TestBlankOverridesBecomeNil(t *testing.T) {
	...
	if got := models.Deref(s.DefaultSavedataOverride); got != "/tmp/x" {
...
func TestRoundtripJSONIsCamelCase(t *testing.T) {
	...
	if models.Deref(back.SteamRootOverride) != "/steam" {
		t.Fatalf("got %+v", back)
	}
}
...
func TestSaveAndLoadRoundtrip(t *testing.T) {
	...
	if models.Deref(want.GameInstallOverride) != "/games/RimWorld" {
		t.Fatalf("update should normalise, got %+v", want)
	}
	if got := Load(); models.Deref(got.GameInstallOverride) != "/games/RimWorld" {
