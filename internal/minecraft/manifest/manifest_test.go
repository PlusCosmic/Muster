...
const good = `{"packs":[{"id":"frontier","name":"Frontier","pack":"https://pack.example/abc/pack.toml",
  "recommended":{"minMemoryMb":4096,"maxMemoryMb":8192,"args":["-XX:+UseZGC"]},"server":"play.example"}]}`

func TestParseGood(t *testing.T) {
	m, err := Parse([]byte(good))
	if err != nil || len(m.Packs) != 1 || m.Packs[0].Recommended.MaxMemoryMb != 8192 || m.Packs[0].Server != "play.example" {
		t.Fatalf("%+v %v", m, err)
	}
}

func TestParseReadsLegacyJavaKey(t *testing.T) {
	m, err := Parse([]byte(`{"packs":[{"id":"a","name":"A","pack":"https://x/p.toml","java":{"maxMemoryMb":6144,"args":["-XX:+UseG1GC"]}}]}`))
	if err != nil || m.Packs[0].Recommended.MaxMemoryMb != 6144 || len(m.Packs[0].Recommended.Args) != 1 || m.Packs[0].LegacyJava != nil {
		t.Fatalf("%+v %v", m.Packs[0], err)
	}
}
...
func TestParseNormalisesEmpty(t *testing.T) {
	...
	if m.Packs[0].Recommended.Args == nil {
...
func TestParseRejects(t *testing.T) {
	cases := map[string]string{
		...
		"min > max": `{"packs":[{"id":"a","name":"x","pack":"https://x/p.toml","recommended":{"minMemoryMb":9,"maxMemoryMb":8}}]}`,
		"not json":  `nope`,
		"arg space": `{"packs":[{"id":"a","name":"x","pack":"https://x/p.toml","recommended":{"args":["-Dcache.dir=C:\\Program Files\\Cache"]}}]}`,
	}
