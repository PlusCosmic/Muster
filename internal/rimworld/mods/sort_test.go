...
func TestBreaksCyclesWithAWarningAndKeepsEveryMod(t *testing.T) {
	...
	if core.Deref(cycles[0].PackageID) != "cyc.alpha" {
...
func TestEmitsTheOtherFourWarningKinds(t *testing.T) {
	...
	if len(missing) != 1 || core.Deref(missing[0].PackageID) != "zed.zebra" || !contains(missing[0].Message, "missing.dep") {
		t.Fatalf("got %+v", missing)
	}
	incompatible := kinds(r, "incompatible")
	if len(incompatible) != 1 || core.Deref(incompatible[0].PackageID) != "con.bar" {
		t.Fatalf("mutual declarations report once: %+v", incompatible)
	}
	version := kinds(r, "versionMismatch")
	if len(version) != 1 || core.Deref(version[0].PackageID) != "old.mod" {
		t.Fatalf("got %+v", version)
	}
	unknown := kinds(r, "unknownMod")
	if len(unknown) != 1 || core.Deref(unknown[0].PackageID) != "not.installed" {
