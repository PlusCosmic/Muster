...
func rulesPath() string { return filepath.Join(paths.CacheRoot(), rulesFile) }
func metaPath() string  { return filepath.Join(paths.CacheRoot(), metaFile) }
...
func LoadCached() *RulesDb {
	...
	if err != nil {
		log.Printf("muster: discarding corrupt rules cache (%v)", err)
		return nil
	}
	return db
}
...
func store(body string, etag *string) error {
	if err := appdir.EnsureDir(paths.CacheRoot()); err != nil {
		return err
	}
	if err := os.WriteFile(rulesPath(), []byte(body), 0o644); err != nil {
		return fmt.Errorf("%s: %w", rulesPath(), err)
	}
	return writeMeta(rulesMeta{FetchedAtMs: core.Int64(time.Now().UnixMilli()), Etag: etag})
}

func touchMeta(etag *string) {
	meta := loadMeta()
	meta.FetchedAtMs = core.Int64(time.Now().UnixMilli())
	if etag != nil {
		meta.Etag = etag
	}
	if err := writeMeta(meta); err != nil {
		log.Printf("muster: %v", err)
	}
}
...
func statusOf(db *RulesDb) models.RulesDbStatus {
	...
	if fetched == nil && db.Timestamp != nil {
		fetched = core.Int64(*db.Timestamp * 1000)
	}
...
func RefreshRulesDb() (models.RulesDbStatus, error) {
	...
	req.Header.Set("User-Agent", "Muster/"+version.Version)
	if meta.Etag != nil {
	...
	etag := core.Str(resp.Header.Get("ETag"))
...
func RulesForSort() *RulesDb {
	...
	if _, err := RefreshRulesDb(); err != nil {
		...
		log.Printf("muster: community rules unavailable (%v)", netErr)
		return nil
	}
	return LoadCached()
}
