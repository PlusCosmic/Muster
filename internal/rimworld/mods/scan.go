...
func toModInfo(about AboutData, dir string, source models.ModSource, workshopID string) models.ModInfo {
	...
	return models.ModInfo{
		...
		Path:              dir,
		Source:            source,
		SteamWorkshopID:   core.Str(workshopID),
		SupportedVersions: core.NonNil(about.SupportedVersions),
		Dependencies:      core.NonNil(about.Dependencies),
		LoadAfter:         core.NonNil(about.LoadAfter),
		LoadBefore:        core.NonNil(about.LoadBefore),
		ForceLoadAfter:    core.NonNil(about.ForceLoadAfter),
		ForceLoadBefore:   core.NonNil(about.ForceLoadBefore),
		IncompatibleWith:  core.NonNil(about.IncompatibleWith),
	}
}
...
func ScanModContainer(parent string, source models.ModSource) []models.ModInfo {
	entries, err := os.ReadDir(parent)
	if err != nil {
		log.Printf("muster: cannot read %s: %v", parent, err)
		return nil
	}
	var dirs []string
	for _, e := range entries {
	...
	for _, dir := range dirs {
		...
		switch {
		case err != nil:
			log.Printf("muster: skipping malformed mod: %v", err)
		case m != nil:
			out = append(out, *m)
		}
	}
...
func ListInstalled() ([]models.ModInfo, error) {
	p := paths.Detect()
	return ScanAll(core.Deref(p.GameInstall), p.WorkshopDirs), nil
}
