...
func TestServiceRoundTrip(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Join(root, "rimworld", "cache"), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(filepath.Join(root, "rimworld", "cache", "communityRules.json"), []byte(`{"rules":{}}`), 0o644); err != nil {
		t.Fatal(err)
	}

	var app Service

	s, err := app.UpdateSettings(models.Settings{SteamRootOverride: core.Str("  ")})
	...
	if err != nil || detected.ProfilesDir != filepath.Join(root, "rimworld", "profiles") || detected.WorkshopDirs == nil {
