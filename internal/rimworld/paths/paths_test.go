...
func TestRimWorldLayoutSitsUnderGameRoot(t *testing.T) {
	t.Setenv("MUSTER_DATA_DIR", "")
	if Root() != appdir.GameRoot(appdir.RimWorld) || filepath.Base(Root()) != "rimworld" {
		t.Fatalf("got %s", Root())
	}
	if filepath.Base(ProfilesRoot()) != "profiles" || !strings.HasPrefix(ProfilesRoot(), Root()) {
		t.Fatalf("got %s", ProfilesRoot())
	}
	if CacheRoot() != filepath.Join(Root(), "cache") || ProfileDir("x") != filepath.Join(ProfilesRoot(), "x") {
		t.Fatalf("%s / %s", CacheRoot(), ProfileDir("x"))
	}
}

func TestDataRootOverride(t *testing.T) {
	dir := t.TempDir()
	t.Setenv("MUSTER_DATA_DIR", dir)
