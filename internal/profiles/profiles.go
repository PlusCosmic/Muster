...
// RegistryPath is `<data>/rimforge/registry.json`.
func RegistryPath() string { return filepath.Join(appdir.DataRoot(), "registry.json") }
...
func takenIDs(reg *Registry) map[string]bool {
	...
	if entries, err := os.ReadDir(appdir.ProfilesRoot()); err == nil {
...
func SaveRegistry(reg *Registry) error {
	if err := appdir.EnsureDir(appdir.DataRoot()); err != nil {
		return err
	}
	if reg.Profiles == nil {
		reg.Profiles = []Meta{}
...
func CountActiveModsInXML(body string) int {
	...
	if err != nil {
		log.Printf("rimforge: malformed ModsConfig.xml: %v", err)
		return 0
	}
...
func copyTree(src, dst string, depth int) error {
	...
	for _, e := range entries {
		...
		if err != nil {
			log.Printf("rimforge: skipping %s (%v)", from, err)
			continue
		}
		switch {
		case info.IsDir():
...
func Create(name string) (models.Profile, error) {
	...
	if err := appdir.EnsureDir(appdir.ProfilesRoot()); err != nil {
		return models.Profile{}, err
	}
	...
	version := models.Deref(paths.Detect().GameVersion)
...
func ImportDefault(name string) (models.Profile, error) {
	...
	source := models.Deref(paths.Detect().DefaultSavedata)
	if source == "" {
	...
	if !copiedAny {
		log.Printf("rimforge: %s had no Config/Saves/Scenarios to import", source)
	}
...
func TouchLastPlayed(id string) error {
	...
	reg.Profiles[i].LastPlayedAtMs = models.Int64(NowMs())
	return SaveRegistry(reg)
}
