...
func TestCrudRoundtripAgainstScratchDataDir(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(appdir.ProfileDir(a.ID), "Config", "ModsConfig.xml")); err != nil {
		t.Fatal(err)
	}
	...
	saves := filepath.Join(appdir.ProfileDir(a.ID), "Saves")
	...
	if _, err := os.Stat(filepath.Join(appdir.ProfileDir(cloned.ID), "Saves", "Colony.rws")); err != nil {
		t.Fatal(err)
	}
	...
	for _, id := range []string{a.ID, b.ID, cloned.ID} {
		...
		if _, err := os.Stat(appdir.ProfileDir(id)); !os.IsNotExist(err) {
...
func TestImportDefaultCopiesSymlinkedSourceWithoutMutatingIt(t *testing.T) {
	...
	dir := appdir.ProfileDir(profile.ID)
