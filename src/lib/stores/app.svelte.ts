...
// that immediately toasts its own failure); nothing else talks to Tauri.
...
class AppStore {
  ...
  async createShortcut(id: string): Promise<void> {
    this.busyProfileId = id;
    try {
      const path = await backend.createShortcut(id);
      toastSuccess('Shortcut created', path);
    } catch (e) {
      toastError('Could not create the shortcut', e);
    } finally {
      this.busyProfileId = null;
    }
  }
