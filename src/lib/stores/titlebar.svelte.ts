...
// frontend preference (localStorage), applied by asking the Tauri window to
// toggle decorations — useful under tiling WMs where the bar is dead space.
...
function stamp(shown: boolean) {
  // No-op outside Tauri (vite in a browser, mock mode).
  if (typeof window === 'undefined' || !('__TAURI_INTERNALS__' in window)) return;
  getCurrentWindow()
    .setDecorations(shown)
