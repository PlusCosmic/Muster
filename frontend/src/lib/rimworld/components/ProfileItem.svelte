<script lang="ts">
  ...
  } from '$lib/rimworld/actions';
  import { app } from '$lib/rimworld/stores/app.svelte';
  import { absoluteTime, plural, relativeTime } from '$lib/shell/format';
  import type { Profile } from '$lib/rimworld/types';
  import ContextMenu, { type MenuItem } from '$lib/shell/components/ContextMenu.svelte';
  ...
  const items: MenuItem[] = $derived([
    { label: 'Launch RimWorld', icon: 'play', onselect: () => launchProfileFlow(profile.id) },
    {
      ...
      onselect: () => cloneProfileFlow(profile.id)
    },
